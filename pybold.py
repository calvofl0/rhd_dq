...
class uio_struct_item(object):
    __slots__ = ['name', 'desc', 'unit', 'value', '__link', 'record', '_parent']
    @property
    def self(self):
        return self
...
first one, etc.
        ...
        if (self._loc != ''):
            ...
            if (mode=='rename' and self[self._loc]._loc==''):
                ...
                if ident != '' :
                    ...
                    labels = uio_reader_module.label.tostring().strip().decode('latin-1')
        ...
        if   mode == 'up' : return
            ...
            else : ident0 = uio_reader_module.label.tostring().strip().decode('latin-1')
            ...
            label = uio_reader_module.label.tostring().strip().decode('latin-1')
            name = uio_reader_module.name0.tostring().strip().decode('latin-1')
            unit = uio_reader_module.unit0.tostring().strip().decode('latin-1')
            qtype = uio_reader_module.qtype.tostring().strip().decode('latin-1')
            ...
            vstrip = np.vectorize(lambda s : s.strip().decode('latin-1'), otypes=[object])
            if ndim == 1 :
                if n1 <= m1 :
                    if qtype == self.__intena:
                        ...
                        try:
                            item.value = uio_reader_module.char0.item().strip().decode()
                        except:
                    ...
                    if qtype == self.__intena:
                        ...
                        item.value = vstrip(np.apply_along_axis(np.ndarray.tostring,-1,uio_reader_module.char1D))
            elif ndim == 2 :
                if qtype == self.__intena:
            ...
            if qtype == self.__intena:
                ...
                item.value = uio_reader_module.char1D.item().strip().decode('latin-1')
