...
def simulate(*args, **kwargs):
	...
	for key in args:
		...
		if(not key in meshes):
			if(hasattr(key,'xb1') and hasattr(key,'xb2') and hasattr(key,'xb3')):
				meshes += (key,)
		if(hasattr(key,'xb1') and hasattr(key,'xb2') and hasattr(key,'xb3')):
			mesh_id += (np.where(np.array(meshes)==key)[0].item(),)
		else:
			mesh_id += (None,)
		index += 1
	...
	for mesh in meshes:
		...
		if(dim[index]==2):
			...
			for i in range(3):
				...
				if centre[i]>=0 :
					if box[i]>=0 :
						id_min[i]=max(id_min,int(np.ceil(centre[i]-box[i]/2.)))
						id_max[i]=min(id_max,1+int(np.floor(centre[i]+box[i]/2.)))
						if max_sz[i]>=0 :
	...
	for key in values:
		if type(key) == tuple:
			print(np.prod([np.array(np.shape(key[0].value))-np.array([1,0,0]),np.array(np.shape(key[1].value))-np.array([0,1,0]),np.array(np.shape(key[2].value))-np.array([0,0,1])],axis=1))
			print(mesh_cells[mesh_id0[index]])
			...
			box1 = box1[id_min[0]:id_max[0]:mesh_step[mesh_id0[index]][0],id_min[1]:id_max[1]:mesh_step[mesh_id0[index]][1],id_min[2]:id_max[2]:mesh_step[mesh_id0[index]][2]]
			box2 = box2[id_min[0]:id_max[0]:mesh_step[mesh_id0[index]][0],id_min[1]:id_max[1]:mesh_step[mesh_id0[index]][1],id_min[2]:id_max[2]:mesh_step[mesh_id0[index]][2]]
			box3 = box3[id_min[0]:id_max[0]:mesh_step[mesh_id0[index]][0],id_min[1]:id_max[1]:mesh_step[mesh_id0[index]][1],id_min[2]:id_max[2]:mesh_step[mesh_id0[index]][2]]
			box=np.empty(np.size(box1)+np.size(box2)+np.size(box3), order='F', dtype=arrtype)
			box[0::3] = box1.flatten()
			box[1::3] = box2.flatten()
			box[2::3] = box3.flatten()
			trash+=(box,)
			...
			if dim[mesh_id0[index]] == 2:
				...
				box=np.array(key.value[id_min[0]:id_max[0]:mesh_step[mesh_id0[index]][0],id_min[1]:id_max[1]:mesh_step[mesh_id0[index]][1],id_min[2]:id_max[2]:mesh_step[mesh_id0[index]][2]], order='F', dtype=arrtype)
				trash+=(box,)
